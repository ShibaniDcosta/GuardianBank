...
const transfer = async (req, res, next) => {
  ...
  try {
    ...
    //   balanceTransfered,
    // };
    // Log pending 'out' transactions in sending user's account
    sendingAccount.out.push({
      to: receivingAccount.id,
      balance_transfered: balanceTransfered,
      status: 'pending'
    ...
    const updatedSendingTransaction = sendingAccount.out[sendingAccount.out.length - 1];
    const updatedReceivingTransaction = receivingAccount.in[receivingAccount.in.length - 1];


    // Update each transaction with the corresponding ID from the other account
    updatedSendingTransaction.correspondingTransactionId = updatedReceivingTransaction._id;

    console.log("updatedSendingTransaction", updatedSendingTransaction)
    //updatedReceivingTransaction.correspondingTransactionId = updatedSendingTransaction._id;

    // Save the updates again
    await sendingAccount.save();
    await receivingAccount.save();
    ...
    // res.status(200).json({
    //   message: "Transaction logged as pending.",
    //   details: {
    //     from: sendingAccount.id,
    //     to: receivingAccount.id,
...
const sendOtp = async (req, res) => {
  const userEmail = req.user.email; // Ensure req.user is populated with user data
  const otp = Math.floor(100000 + Math.random() * 900000); // Generate a 6-digit OTP
  const ttl = 5 * 60 * 1000; // 5 minutes validity
  const expires = Date.now() + ttl; // Expiry time calculation

  // Simpler approach to store OTP; normally you would want a more secure way to handle this
  otpStore[userEmail] = { otp: otp, expires: expires };

  try {
    // Send email with OTP
    await sendEmail(
      userEmail,
      'One Time Password - Guardian Bank',
      `Your One-Time Password (OTP) is ${otp}. Please use it promptly to complete your authentication or transaction.`
    );
...
const verifyOtp = async (req, res) => {
  ...
  const storedData = otpStore[userEmail];

  if (!storedData) {
    console.log("No otp is getting stoered")
    return res.status(400).json({ success: false, message: "No OTP stored." });
  }

  // Check if OTP is correct and not expired
  if (parseInt(storedData.otp) === parseInt(otp) && storedData.expires > Date.now()) {
