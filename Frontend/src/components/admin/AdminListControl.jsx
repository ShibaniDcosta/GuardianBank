...
const AdminListControl = ({ adminsList }) => {
      ...
      >
        ...
        >
          <form onSubmit={(event) => handleRemoving(event, admin._id)}>
            <FormButton
              text={{ default: "Remove" }}
              bgColor={["bg-indigo-500", "bg-indigo-500", "bg-indigo-500"]}
  ...
  return (
    <div className="max-w-5xl w-full">
      <h3 className="text-2xl my-10 p-3 text-center font-bold bg-indigo-100 text-gray-900  border-blue-800 rounded shadow">
        Employees List ({filteredAdmins && filteredAdmins.length})
      </h3>

      {/*search admins with name*/}
      <div className="flex justify-center items-center flex-wrap md:flex-nowrap gap-4 mb-6 p-4 bg-indigo-100 rounded-md  border-blue-800">
        <label
          htmlFor="searchQuery"
      ...
      {searchQuery && filteredAdmins.length === 0 && !isLoading && (
        <div className="text-gray-800 text-center font-bold my-4 py-4 px-2 border-l-4 ">
